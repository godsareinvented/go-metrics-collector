...
func (s *Server) Start() {
	...
	case <-(*s.ctx).Done():
		...
		//  Но остановка сервера в текущем варианте останавливает хендлеры моментально, не давая им возможности корректно обработать завершение родительного контекста (точно?).
...
func (s *Server) createAndConfigureRouter() {
	...
	s.router.Route("/", func(router chi.Router) {
		...
		s.router.Route("/updates", func(router chi.Router) {
			router.Post("/", handler.UpdateMetricBatchMetric(*s.ctx))
		})
