...
2. [metric](internal/dto/metric.go:#L6) Переписать констреинты для больщего соответствия сущности
...
4. Теоретически, для значений метрик с типом ```Counter``` может произойти переполнение переменной
